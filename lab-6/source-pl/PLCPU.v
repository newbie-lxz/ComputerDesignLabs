...
module PLCPU(
 ...
 // instantiation of pc unit
	PC U_PC(.clk(~clk), .rst(reset), .NPC(PC_next), .PC(PC_out) );
	NPC U_NPC(.PC(PC_out), .NPCOp(EX_NPCOp),
	          .JumpPC(EX_pc), .IMM(EX_immout), .RS1(EX_RD1), .NPC(NPC));
	EXT U_EXT(
...
// MUX Gate 
    ...
    wire control_flush;
    assign control_flush = (EX_NPCOp != `NPC_PLUS4);

    wire rs1_hazard;
    wire rs2_hazard;
    wire stall;

    assign rs1_hazard = (rs1 != 5'b0) &&
                        ((EX_RegWrite  && (EX_rd  == rs1)) ||
                         (MEM_RegWrite && (MEM_rd == rs1)) ||
                         (WB_RegWrite  && (WB_rd  == rs1)));

    assign rs2_hazard = (rs2 != 5'b0) &&
                        ((EX_RegWrite  && (EX_rd  == rs2)) ||
                         (MEM_RegWrite && (MEM_rd == rs2)) ||
                         (WB_RegWrite  && (WB_rd  == rs2)));

    assign stall = !control_flush && (rs1_hazard || rs2_hazard);
    assign PC_next = stall ? PC_out : NPC;
...
//-----pipe registers--------------
    ...
    assign IF_ID_in = stall ? IF_ID_out :
                      (control_flush ? 64'b0 : IF_ID_raw_in);
    ...
    assign ID_EX_in = (stall || control_flush) ? 194'b0 : ID_EX_raw_in;
