...
## 提交要求
提交前请先执行 .\build.bat clean 或 make clean，确保不提交生成物。
提交 `lab-2` 目录，至少包含：

- `README.md`
- `modelsim_guide.md`
- `source/` 中的 Verilog 源码、`rv32_pl_sim.dat`、`Makefile`、`build.bat`、`build.py`、`source/README.md`
...
1. 是否能在 `source` 目录执行 `.\build.bat` 或 `python .\build.py run`。
2. 是否能在 Linux / WSL 中执行 `make run`。
3. 是否生成 `plcpu_sim.vcd`。
4. 是否能在波形中找到 `clk`、`rstn`、`PC`、`instr`、`RFWr`、`A3`、`WD`。
5. 学生是否能说明 `plcomp_tb.v` 是仿真顶层，`rv32_pl_sim.dat` 依赖运行目录相对路径。
